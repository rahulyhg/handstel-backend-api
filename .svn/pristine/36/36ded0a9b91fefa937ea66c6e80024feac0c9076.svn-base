...
function login()
{
	...
	$school = $user_service->getUserSchool($user['user_id']); 	
	$child = $user_service->getUserChild($user_id);
	...
	$payload = array('token'=>$token,'name'=>$user['user_name'],'user_id'=>$user['user_id'],'school_id'=>$school['school_id'],'child_id'=>$child['child_id'],'profile_pic'=>$user['profile_pic']);
	echo json_encode(new StandardResponse(SUCCESS,200,$payload));
	return;
	}
...
public function createGroup($group_name)
{
	...
	try
	{
	$group_service -> joinGroup($user_id,$group_id,$child['child_id']);
	}
	catch(Exception $e)
	{
		throw new Exception($e);
...
public function getUserProfile($user_id)
{
	...
	$payload = array('user_name' => $user['user_name'],'profile_pic' => $profile_pic,'groups' =>$groups,'school_id'=>$school['school_id'],'school_name'=>$school['school_name'],'child_name'=>$child['child_name']);
	echo json_encode(new StandardResponse(SUCCESS,200,$payload));
	return;
}
...
public function editProfile($user_id)
{
	...
	$child  = $user_service->getUserChild($user_id);
	$stored_path = stripcslashes($stored_path);
	$payload =  array('user_name' => $user_name,'profile_pic'=>$stored_path,'child_name'=>$child['child_name'],'school_name'=>$school['school_name']);
	echo json_encode(new StandardResponse(SUCCESS,200,$payload));
	return; 
...
public function pushNotificationKeys()
{
	global $app;
	$req = $app->request();
	...
	foreach($user_ids as $user_id)
	{
		$user = $user_service->getUserbyUserID($user_id);
		$data = new stdClass();
		$data->gcm_key =  $user['gcm_key'];
		$data->apns_key = $user['apns_key'];
		array_push($push_keys,$data);
	}
	
	$payload =  array('push_keys' => $push_keys);
	echo json_encode(new StandardResponse(SUCCESS,200,$payload));
	return; 
}
...
public function getSchoolNotifications($user_id)
{
	...
	foreach ($groups as $group) 
	{
		...
		foreach ($notifications as $notification) 
		{
			...
			array_push($group_notification,array('group_id'=>$group_id,'group_name'=>$group_name,'message'=>$message,'notification_date'=>$notification_date));
			 
		}
		
		array_push($group_notifications,array('group'=>$group_notification));	  
	
	}

	$payload = array('notifications' => $group_notifications);
	echo json_encode(new StandardResponse(SUCCESS,200,$payload));
	return; 
...
public function pushSchoolNotification()
{
	...
	foreach ($groups as $group)
	{
 	...
 	$gcm_registrations = array_unique($gcm_registrations);
 	$unique_gcm = array();
 	foreach ($gcm_registrations as $key => $value) {
 		array_push($unique_gcm, $value);
 	}

 	// $payload = array('gcm_registrations'=>$unique_gcm);
	$payload = $gcm->send_notification($unique_gcm,array('message'=>$message));//this works 
...
try
{
move_uploaded_file($_FILES["file"]["tmp_name"], dirname(__FILE__)."/varuna.jpg");
echo json_encode(array('sucessful'=>"1"));
}
catch(Exception $e)
{
