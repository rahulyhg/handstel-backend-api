...
{
     $conn = DBConnection::getInstance()->getConnection();
     $sql = "select user_id,user_name,profile_pic,gcm_key from user where user_id in (select user_id from group_users where group_id=:group_id)";
     try 
   {
        $stmt = $conn->prepare($sql);
        $stmt->bindParam("group_id", $group_id);
...
public function getUserGroup($user_id,$group_id)
{
    ...
    try 
   {
        ...
        $stmt->bindParam("group_id", $group_id);
        $stmt->execute();
        $user_group = $stmt->fetchAll(\PDO::FETCH_OBJ);
        $conn = null;
         if(!empty($user_group))
            return $user_group[0];
   } 
